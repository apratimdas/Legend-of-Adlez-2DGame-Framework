...
using PlayerType = PlayerManagerScriptFinal.PlayerType;

public class PlayerControllerScriptFinal : MonoBehaviour
{
    ...
    string attackName = "Sword Slash";

    public void Awake()
    {
        if (playerType == PlayerType.MAGE)
        {
            attackShouts = new string[] { "BURN!!", "Fireball!", "Perish!" };
            attackName = "StaffSwing";
        }
    ...
    void Update()
    {
        ...
        rotationlock = swordAnimator.GetCurrentAnimatorStateInfo(0).IsName(attackName) ? true : false;
